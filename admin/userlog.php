...
<form onsubmit="return searchSubmit()" method="GET" class="form-inline">
  <div class="form-group">
    <label>搜索</label>
	<select name="column" class="form-control"><option value="uid">UID</option></select>
  </div>
  <div class="form-group">
    <input type="text" class="form-control" name="value" placeholder="搜索内容（0为管理员）">
  </div>
  <button type="submit" class="btn btn-primary">搜索</button>
  <a href="javascript:listTable('start')" class="btn btn-default" title="刷新管理员日志"><i class="fa fa-refresh"></i></a>
</form>
</div>
	...
	$("#listTable").bootstrapTable({
		...
		columns: [
			...
			},
			{
				...
				title: 'UID',
				formatter: function(value, row, index) {
					return '<b>'+(value>0?'<a href="./ulist.php?column=uid&value='+value+'" target="_blank">'+value+'</a>':'管理员')+'</b>';
				}
			},
			{
				field: 'type',
